...
func (v VarInt) Bytes() []byte {
	b := make([]byte, 9)
	if v < 0xfd {
		b[0] = byte(v) //nolint:gosec // varint value range already validated
		return b[:1]
	}
	if v < 0x10000 {
		b[0] = 0xfd                                      //nolint:gosec // varint value range already validated
		binary.LittleEndian.PutUint16(b[1:3], uint16(v)) //nolint:gosec // varint value range already validated
		return b[:3]
	}
	if v < 0x100000000 {
		b[0] = 0xfe                                      //nolint:gosec // varint value range already validated
		binary.LittleEndian.PutUint32(b[1:5], uint32(v)) //nolint:gosec // varint value range already validated
		return b[:5]
	}
	b[0] = 0xff //nolint:gosec // varint value range already validated
...
func (v *VarInt) ReadFrom(r io.Reader) (int64, error) {
	...
	default:
		*v = VarInt(binary.LittleEndian.Uint16([]byte{b[0], 0x00})) //nolint:gosec // single byte value
		return 1, nil
	}
}
