...
to verify validity.  This package handles all encoding and decoding of message headers.

To achieve this, there is a generic interface for Bitcoin messages named
...
messages, all the details of marshaling and unmarshalling to and from the
wire using bitcoin encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the bitcoin messages are
...
the appropriate interactions, see the official bitcoin protocol wiki entry
...
and write bitcoin messages.  The following sections provide a quick overview
